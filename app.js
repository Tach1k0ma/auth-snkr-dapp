...
function createSnkrDiv(sku, upc, image, sneakerId){
    ...
    $cardContainer = $('<div>').attr('class', 'card snkr-style float-left');

    $cardBody = $('<div>').attr('class', 'card-body');

    $snkrImg = snkrImgGen(`images/${image}`);

    $h5 = $('<h5>').attr('class', 'card-title').text(name);

    var aft_id_text = afterIdText(dogId);

    $p_snkr_id = $('<p>').attr('class', 'card-text').text(`${name} is the ${dogId}${aft_id_text} CryptoDog to ever be created.`);

    $p = $('<p>').attr('class', 'card-text').text(`${name} is ${age} years old.`);

    $cardBody.append($snkrImg, $h5, $p_snkr_id, $p);
...
App = {
    ...
    mintSnkr: function(event) {
        event.preventDefault();
        
        console.log($snkr_image_url_INPUT.val(), $sku_INPUT.val(), $upc_INPUT.val());

        App.contracts.Snkr.deployed().then(function(instance) {
            snkrInstance = instance;
            //Is snkrInstance = SnkrInstance? 

            return snkrInstance.mint($snkr_image_url_INPUT.val(), $sku_INPUT.val(), $upc_INPUT.val());
        }).then(function(result) {
